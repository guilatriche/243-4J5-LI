...
class LEDControlUI:
    def __init__(self, stdscr, touch_reader: TouchReader, event_queue: Queue, mqtt_config: dict):
        ...
        # Topics pour écouter l'état réel (retour du bouton physique)
        ...
        # Animation de fond (Balle rebondissante)
        self.ball = {
            'x': 5,
            'y': 5,
            'dx': 1,
    ...
    def _on_mqtt_connect(self, client, userdata, flags, rc):
        ...
        if rc == 0:
            ...
            # S'abonner aux topics d'état des LEDs (pour sync avec bouton physique)
    ...
    def _on_mqtt_message(self, client, userdata, msg):
        ...
        # Mise à jour de l'état local si on reçoit un message d'état LED
        if hasattr(self, 'led1_state_topic') and topic == self.led1_state_topic:
            self.led1_state = (payload == "ON")
        elif hasattr(self, 'led2_state_topic') and topic == self.led2_state_topic:
    ...
    def _init_colors(self):
        ...
        curses.init_pair(10, curses.COLOR_MAGENTA, curses.COLOR_BLACK) # Balle (Magenta sur noir)

        # Arc-en-ciel (Backgrounds)
        curses.init_pair(20, curses.COLOR_WHITE, curses.COLOR_RED)
        curses.init_pair(21, curses.COLOR_BLACK, curses.COLOR_YELLOW)
        curses.init_pair(22, curses.COLOR_BLACK, curses.COLOR_GREEN)
        curses.init_pair(23, curses.COLOR_BLACK, curses.COLOR_CYAN)
        curses.init_pair(24, curses.COLOR_WHITE, curses.COLOR_BLUE)
        curses.init_pair(25, curses.COLOR_WHITE, curses.COLOR_MAGENTA)

    def _draw_rainbow_background(self, h, w):
        """Dessine un fond arc-en-ciel animé"""
        colors = [20, 21, 22, 23, 24, 25]
        stripe_height = 2
        
        for row in range(h):
            color_idx = ((row // stripe_height) + self.rainbow_offset) % len(colors)
            attr = curses.color_pair(colors[color_idx])
            try:
                self.stdscr.addstr(row, 0, " " * (w - 1), attr)
            except curses.error:
                pass
        
        self.rainbow_tick += 1
        if self.rainbow_tick % 5 == 0:  # Vitesse
            self.rainbow_offset += 1

    def _update_and_draw_ball(self, h, w):
        """Met à jour et dessine la balle rebondissante en arrière-plan"""
        ...
        if self.ball['x'] <= 0 or self.ball['x'] >= w - 1:
            self.ball['dx'] *= -1
            self.ball['x'] = max(0, min(w - 1, self.ball['x'])) # Clamp
        
        if self.ball['y'] <= 0 or self.ball['y'] >= h - 1:
            self.ball['dy'] *= -1
            self.ball['y'] = max(0, min(h - 1, self.ball['y'])) # Clamp

        # Dessin (si dans les limites)
        if 0 <= self.ball['y'] < h and 0 <= self.ball['x'] < w:
            try:
                self.stdscr.attron(curses.color_pair(10) | curses.A_BOLD)
                self.stdscr.addstr(int(self.ball['y']), int(self.ball['x']), self.ball['char'])
    ...
    def _draw(self):
        ...
        # Dessiner le fond arc-en-ciel
        self._draw_rainbow_background(h, w)

        # Dessiner l'animation de balle par dessus
        self._update_and_draw_ball(h, w)
